...
use MOM_error_handler,         only : MOM_error, FATAL, WARNING, is_root_pe, NOTE
...
type, public :: hor_visc_CS ; private
  ...
  real    :: Re_Kh           !! If nonzero, the harmonic coefficient is scaled
                             !< so that the harmonic Reynolds number is equal to this.
  ...
  logical :: zero_out_Leith  !< If true, zero out the viscous contribution of Leith.
  ...
  real ALLOCABLE_, dimension(NIMEMB_PTR_,NJMEMB_PTR_) :: &
    ...
    Re_Kh_const_xy      !< Harmonic metric-dependent constants [L ~> m]
  ...
  integer :: id_FrictWork = -1, id_FrictWorkIntz = -1, id_Frictflux = -1, id_FrictWork_old = -1
...
subroutine horizontal_viscosity(u, v, h, uh, vh, diffu, diffv, MEKE, VarMix, G, GV, US, &
  ...
  real, dimension(SZI_(G),SZJ_(G),SZK_(GV)) :: &
    ...
    FrictWork_old, &     ! work done by MKE dissipation mechanisms [R L2 T-3 ~> W m-2]
  ...
  real :: div_xx_sq  ! Square of divergence [T-2 ~> s-2]
...
!  real :: Sh_F_pow  ! The ratio of shear over the absolute value of f raised to some power and rescaled [nondim]
  ...
  do k=1,nz
    ...
    if (CS%Laplacian) then
      ...
      do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
        if (CS%add_LES_viscosity) then
          ...
          if (CS%Leith_Kh) &
            Kh(i,j) = max(Kh(i,j), zero_out_Leith * CS%Laplac3_const_xx(i,j) * vert_vort_mag(i,j) * inv_PI3)
        endif
      enddo ; enddo
      ...
      if (CS%Re_Kh > 0.0) then
        do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
          KE = 0.125*((u(I,j,k)+u(I-1,j,k))**2 + (v(i,J,k)+v(i,J-1,k))**2)
          Kh(i,j) = sqrt(KE) * CS%Re_Kh_const_xx(i,j)
        enddo ; enddo
      endif
    ...
    if (CS%biharmonic) then
      ...
      if ((CS%Smagorinsky_Ah) .or. (CS%Leith_Ah)) then
        ...
        if (CS%Leith_Ah) then
          do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
            AhLth = zero_out_Leith * CS%Biharm6_const_xx(i,j) * abs(Del2vort_h(i,j)) * inv_PI6
    ...
    if (CS%Laplacian) then
      ...
      if (CS%Leith_Kh) then
        if (CS%add_LES_viscosity) then
          ...
          do J=js-1,Jeq ; do I=is-1,Ieq
            Kh(I,J) = max(Kh(I,J), zero_out_Leith * CS%Laplac3_const_xy(I,J) * vert_vort_mag(I,J) * inv_PI3)
          enddo ; enddo
        endif
      endif
      ...
      if (CS%Re_Kh > 0.0) then
        do J=js-1,Jeq ; do I=is-1,Ieq
          ...
          Kh(I,J) = sqrt(KE) * CS%Re_Kh_const_xy(I,J)
        enddo ; enddo
      endif
    ...
    if (CS%biharmonic) then
      ...
      if (CS%Smagorinsky_Ah .or. CS%Leith_Ah) then
        ...
        if (CS%Leith_Ah) then
          do J=js-1,Jeq ; do I=is-1,Ieq
            AhLth = zero_out_Leith * CS%Biharm6_const_xy(I,J) * abs(Del2vort_q(I,J)) * inv_PI6
            Ah(I,J) = max(Ah(I,J), AhLth)
          enddo ; enddo
        endif
      ...
      if (CS%bound_Kh_with_MEKE) then
          do J=js-1,Jeq ; do I=is-1,Ieq
            ...
            visc_limit_q_frac(I,J,k) = Ah(I,J) / (CS%KS_coef * hrat_min(I,J) * CS%Ah_Max_xy_KS(I,J)) !i,j
            if ((Ah(I,J) >= tmp) .or. (hq(I,J)<=CS%KS_thick)) then  !h(i,j,k)
    ...
    if (find_FrictWork) then ; do j=js,je ; do i=is,ie
      ...
      if (visc_limit_h_flag(i,j,k) > 0) then
       ...
       +0.25*((str_xy(I,J-1)*(vh(i,J-1,k)/(h_v(i,J-1)+h_neglect)*G%IdxCv(i,J-1)+vh(i+1,J-1,k)/(h_v(i+1,J-1)+h_neglect)*G%IdxCv(i+1,J-1)) & 
        ...
        FrictWork_old(i,j,k) = GV%H_to_RZ * ( &
    ...
    if (find_FrictWork .and. allocated(MEKE%mom_src)) then
      ...
      do j=js,je ; do i=is,ie
        MEKE%mom_src_3d(i,j,k) = 0.
        MEKE%mom_src_bh_3d(i,j,k) = 0. !cyc
      enddo ; enddo
      if (MEKE%backscatter_Ro_c /= 0.) then
        do j=js,je ; do i=is,ie
          ...
          MEKE%mom_src_3d(i,j,k) = MEKE%mom_src_3d(i,j,k) + FrictWork(i,j,k) - RoScl*FrictWork_bh(i,j,k)
          MEKE%mom_src_bh_3d(i,j,k) = MEKE%mom_src_bh_3d(i,j,k) + FrictWork_bh(i,j,k) - RoScl*FrictWork_bh(i,j,k)
      ...
      do j=js,je ; do i=is,ie
        ...
        MEKE%mom_src_3d(i,j,k) = MEKE%mom_src_3d(i,j,k) + FrictWork(i,j,k)
        MEKE%mom_src_bh_3d(i,j,k) = MEKE%mom_src_bh_3d(i,j,k) + FrictWork_bh(i,j,k) !cyc
      enddo ; enddo
  ...
  if (CS%id_FrictWork_old>0) call post_data(CS%id_FrictWork_old, FrictWork_old, CS%diag)
...
# include "version_variable.h"
  ...
  call get_param(param_file, mdl, "RE_KH", CS%Re_Kh, &
                 "If nonzero, the harmonic coefficient is scaled "//&
                 "so that the harmonic Reynolds number is equal to this.", &
                 units="nondim", default=0.0, do_not_log=.not.CS%Laplacian)
  ...
  call get_param(param_file, mdl, "ZERO_OUT_LEITH", CS%zero_out_Leith, &
               "If true, zero out the QG Leith viscosity.", &
               default=.false.)
  ...
  if (CS%Laplacian) then
    ...
    if (CS%Re_Kh > 0.0) then
      ALLOC_(CS%Re_Kh_const_xx(isd:ied,jsd:jed)); CS%Re_Kh_const_xx(:,:) = 0.0
      ALLOC_(CS%Re_Kh_const_xy(IsdB:IedB,JsdB:JedB)); CS%Re_Kh_const_xy(:,:) = 0.0
    endif
  endif
  ...
  if (CS%Laplacian) then
   ...
   ! this to be less than 1/3, rather than 1/2 as before.
    ...
    do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
      ...
      if (CS%Re_Kh > 0.0) then
        CS%Re_Kh_const_xx(i,j) = SQRT(grid_sp_h2) / CS%Re_Kh
      endif
    enddo ; enddo
    ...
    do J=js-1,Jeq ; do I=is-1,Ieq
      ...
      if (CS%Re_Kh > 0.0) then
        CS%Re_Kh_const_xy(I,J) = SQRT(grid_sp_q2) / CS%Re_Kh
      endif
    enddo ; enddo
  endif
  if (CS%biharmonic) then
  ...
  CS%id_FrictWork = register_diag_field('ocean_model','FrictWork',diag%axesTL,Time,&
      ...
      'W m-2', conversion=US%RZ3_T3_to_W_m2*US%L_to_Z**2)
  CS%id_FrictWork_old = register_diag_field('ocean_model','FrictWork_old',diag%axesTL,Time,&
      'Integral work done by lateral friction terms in the old form. If GME is turned on, this '//&
      'includes the GME contribution.', &
      'W m-2', conversion=US%RZ3_T3_to_W_m2*US%L_to_Z**2)
...
subroutine hor_visc_end(CS)
  ...
  if (CS%Laplacian) then
    ...
    if (CS%Re_Kh > 0.0) then
      DEALLOC_(CS%Re_Kh_const_xx) ; DEALLOC_(CS%Re_Kh_const_xy)
    endif
  endif
  if (CS%biharmonic) then
